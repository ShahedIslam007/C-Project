...
namespace MyProject
{
    public partial class AdminControlerOne : UserControl
    {
        string cs = ConfigurationManager.ConnectionStrings["dbcs"].ConnectionString;
        public AdminControlerOne()
        {
            InitializeComponent();
            GridView();
        }
        ...
        private void button1_Click(object sender, EventArgs e)
        {
            ...
            string query = "insert into Recruiters values(@name,@email,@phone,@addr,@district,@area,@img)";
        ...
        {
            SqlConnection con = new SqlConnection(cs);
            string query = "update Recruiters set Name=@name, Email=@email, Phone=@phone, Address=@addr, District=@district, Area=@area, Picture=@img where Name=@name";
            SqlCommand cmd = new SqlCommand(query, con);
            cmd.Parameters.AddWithValue("@name", textBox5.Text);
            cmd.Parameters.AddWithValue("@email", textBox4.Text);
            cmd.Parameters.AddWithValue("@phone", textBox3.Text);
        ...
        {
            SqlConnection con = new SqlConnection(cs);
            string query = "delete from Recruiters where Name=@name";
            SqlCommand cmd = new SqlCommand(query, con);
            cmd.Parameters.AddWithValue("@name", textBox5.Text);
