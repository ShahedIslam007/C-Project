...
namespace MyProject
{
    public partial class ApplicantControlerThree : UserControl
    {
        string cs = ConfigurationManager.ConnectionStrings["dbcs"].ConnectionString;
        public ApplicantControlerThree()
        {
            InitializeComponent();
        }
        ...
        {
            SqlConnection con = new SqlConnection(cs);
            string query = "select * from Nearby_Mosque where District=@District and Area=@Area";
        ...
        {
            comboBox2.SelectedItem = dataGridView1.SelectedRows[0].Cells[0].Value.ToString();
