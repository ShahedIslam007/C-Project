...
namespace MyProject
{
    public partial class AdminControlerTwo : UserControl
    {
        string cs = ConfigurationManager.ConnectionStrings["dbcs"].ConnectionString;
        public AdminControlerTwo()
        {
            InitializeComponent();
            GridView();
        }
        ...
        {
            SqlConnection con = new SqlConnection(cs);
            string query = "insert into Applicants values(@name,@email,@phone,@addr,@district,@area,@img)";
        ...
        {
            SqlConnection con = new SqlConnection(cs);
            string query = "update Applicants set Name=@name, Email=@email, Phone=@phone, Address=@addr, District=@district, Area=@area, Picture=@img where Name=@name";
            SqlCommand cmd = new SqlCommand(query, con);
            cmd.Parameters.AddWithValue("@name", textBox5.Text);
            cmd.Parameters.AddWithValue("@email", textBox4.Text);
            cmd.Parameters.AddWithValue("@phone", textBox3.Text);
        ...
        {
            SqlConnection con = new SqlConnection(cs);
            string query = "delete from Applicants where Name=@name";
            SqlCommand cmd = new SqlCommand(query, con);
            cmd.Parameters.AddWithValue("@name", textBox5.Text);
